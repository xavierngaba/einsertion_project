                                    ...
                                    <c:choose>
                                        <c:when test="${message == 'vide'}">
                                            <div class="alert alert-info" style="width: 40%; margin-left: 22%; margin-top: 10px">
                                                ...
                                                Actuellement aucune candidature n'a &eacute;t&eacute; sollicit&eacute; pour cette offre d'emploi sur notre plate-forme
                                            </div>
                                        </c:when>
                                        <c:otherwise>
                                            <table id="example" class="table table-hover table-condensed" cellspacing="0">
                                               ...
                                               <c:forEach items="${ requestScope.OffreEmploi.postulatList}" var="postulat" varStatus="i">
                                                   <c:forEach items="${ requestScope.CandidatList}" var="candidat" varStatus="j">
                                                       <c:if test="${i.index == j.index}">
                                                           <tbody>
                                                                <tr>
                                                                    <td style="font-style: italic; font-size: 20px" class="text-info">
                                                                        <c:out value="${candidat.nomCan}"/> &nbsp; <c:out value="${candidat.prenomCan}"/>  
                                                                    </td>
                                                                    <td>
                                                                        <label style="color: red"> <c:out value="${requestScope.OffreEmploi.poste}"/> </label>
                                                                    </td>
                                                                    <td>
                                                                        <label style="color: red"> <c:out value="${postulat.datePost}"/> </label>
                                                                    </td>
                                                                    <td>
                                                                        <a href="<c:url value="/DetailServlet?action=detailcv&codeCan=${candidat.codeCan}&id=${sessionScope.sessionUtilisateur.code.code}"/>" class="btn-small btn-info" type="submit">
                                                                            <i class="icon-white icon-eye-open"></i> D&eacute;tails...
                                                                        </a>
                                                                    </td>
                                                                    <td>
                                                                        <a href="<c:url value="/AffichageLettreServlet?&codeCan=${candidat.codeCan}&codeEnt=${sessionScope.sessionUtilisateur.code.code}&codeOffreEmp=${requestScope.OffreEmploi.codeOffreEmp}"/>" class="btn-small btn-info" type="submit">
